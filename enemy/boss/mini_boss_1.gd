...
func _physics_process(delta: float) -> void:
	...
	if health<=0:
		...
		$attack1Timer.stop();$attack2Timer.stop()
...
func _on_attack_2_timer_timeout() -> void:
	...
	for i in 4:
			...
			get_parent().add_child(p1)
			match i:
				0:p1.global_position=$mainBody/Marker2D.global_position+Vector2(0,0)
				1:p1.global_position=$mainBody/Marker2D2.global_position+Vector2(0,0)
				2:p1.global_position=$mainBody/Marker2D3.global_position+Vector2(0,0)
				3:p1.global_position=$mainBody/Marker2D4.global_position+Vector2(0,0)
	$attack1Timer.start()
