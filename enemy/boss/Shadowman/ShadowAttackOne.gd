...
@export var throwAttackOneTimer: Timer
...
func TimerUp():
	if shootNumber <= 2 and topMarker:
		...
		get_parent().add_child(blades)
		blades.global_position = topMarker.global_position
	...
	match animatedSprite.animation:
		"Throw":
			...
			if topMarker:
				...
				#shootNumber = shootNumber + 1
				throwAttackOneTimer.start()
				#transition.emit(self, "ShadowStop")
				if stopTimer:
					stopTimer.wait_time = .5


func _on_throw_attack_one_timer_timeout() -> void:
	shootNumber = shootNumber + 1
	animatedSprite.play("Throw")
