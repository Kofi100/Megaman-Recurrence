...
func _process(delta: float) -> void:
	match MenuOptionNo:
		...
		4:$SelectArrow.global_position=(Vector2($ScrollContainer/VBoxContainer/quit.global_position.x - 20, $ScrollContainer/VBoxContainer/quit.global_position.y))
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventAction or event is InputEventKey:
		...
		MenuOptionNo = clampi(MenuOptionNo, 0, 4)
